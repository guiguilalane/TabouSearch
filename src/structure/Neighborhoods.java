...
public class Neighborhoods {
	...
	public Neighborhoods(IntDomain[] domains, int[] solution,
			List<Pair> tabuMoves) {
		...
		// //pour chaque valeur de la solution courante, il y a n-1 autre
		// solution
		// int k = 0; // colonne
		// //pour chaques valeurs des autres solutions, parcourir le domaine
		// possible
		// for(int j = domains[i].min(); j <= domains[i].max(); j++) { //j
		// valeur du domaine de xi
		// //vérifier que la valeur courante (j) n'est pas la solution
		// //auquel cas l'ajouter dans le tableau de solution
		// if(j != sol[i]) {
		// sols[i][k] = j;
		// ++k;
		// // System.out.println("current sol : " + sol[i] + ", possible sol : "
		// + j);
		// }
		// }
		// }
		...
		for (int i = 0; i < sol.length; ++i) {
			cloneDomain[i] = new IntervalDomain(domains[i].min(),
					domains[i].max());
		}
		for (int i = 0; i < sol.length; ++i) {
	...
	public int[] calculateBestCandidate(Pair tabuMove) {
		...
		System.out.println("bloup " + subsets.size());
